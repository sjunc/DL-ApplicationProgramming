...
## 11주차 시계열 (Time Series) 분석   
시계열 자료: 시간의 흐름에 따라 수집된 데이터  
• 예: 하루 단위의 기온 변화, 주가, 월별 판매량 등.  
• 시계열 분석: 시간에 따라 변하는 데이터를 사용하여 추이를 분석함  
• 즉, 추세를 파악하거나 향후 전망 등을 예측하기 위한 용도로시계열 분석을 사용함  

### 시계열의 4대 구성 요소(Components of Time Series)  
불규칙 변동, 추세 변동, 순환 변동, 계절 변동  
   
불규칙 변동(Irregular Variation)  
: **예측 불가능한** 돌발적 요인으로 발생하는 변동  
- 패턴이 없으므로 모형을 만들기 매우 어려움  
예: 갑작스러운 자연재해, 코로나로 인한 주가 급락, 일시적 이벤트  
  
추세 변동(Trend Variation)  
: 장기적인 변화 추세  
: 시간 흐름에 따라 지속적 상승, 하락, 유지 경향을 보이는 것  
- 짧은 시간동안 추세 변동을 찾기 어려움  
- **장기적 구조**를 파악하는 핵심 요소  
예: 전 세계 인구 → 장기 상승, 국내총생산(GDP) 증가, 스마트폰판매량증가  
  
순환 변동(Cyclical Variation)  
: 수년 단위(예: 2-3년)로 반복되는 경제·사회적 요인에 의한 파동적변화  
: 주기는 일정하지 않지만 반복성 존재  
예: 경기 호황 → 불황 → 회복 → 호황  
  
계절 변동(**Season**al Variation)   
: 보통 계절적 영향과 사회적 관습에 따라 1년 주기로 발생하는변동: 주로 계절에 따라 순환하며 변동하는 특성이 있음  
- 계절성을 모델링하면 예측 정확도를 높일 수 있음   
예: 겨울 아이스크림 판매 감소, 여름 전기 사용량 증가, 명절 과일 선물세트 매출 증가   

### 규칙적 시계열 vs 불규칙적시계열
  
시계열 데이터를 잘 분석한다 = 불규칙성을 갖는 시계열 데이터에 특정한 기법이나 모델을 적용해 규칙적 패턴을 찾거나 예측한다  
규칙적 시계열  
• 추세, 계절성, 순환성과 같은 명확한 패턴 존재  
• 트렌드와 분산이 불변하는 시계열 데이터  
• 예:  
월별 전기 사용량(계절성 있음)  
출산율(추세 있음)  
불규칙적 시계열  
• 패턴이 거의 없고 변동성이 큰 데이터  
• 트렌드 혹은 분산이 변화하는 시계열 데이터  
• 예:  
로또 판매량  
하루 단위 갑작스러운 기온 변화  
• 이런 경우 패턴을 부여하는 모델(AR, MA 등)을 사용해 예측 가능하게 만듦  
  
Q: 불규칙적 시계열 데이터를 잘 예측하려면?   
A: 일정한 패턴을 부여해야 함.   
• Q: 규칙성을 부여하는 방법?  
A: 모델을 적용할 수 있음  
- 최근에는 딥러닝을 이용하여 시계열 데이터의 연속성을 추출하는 방법도 사용되고 있음  

### AR, MA, ARMA, ARIMA
x축 시간 y축 종속변수  
• 시계열 분석은 독립변수(시간)를 사용하여 종속변수를 예측하는 것임  
• 불규칙한 데이터라도 과거 값의 패턴 또는 오차 패턴을 학습하면 예측 가능성이 생김  
### AR (자기회귀) AR 모델 (Auto Regression Model, 자기회귀 모델)  
• (idea) "이전의 관측값이 이후 관측값에도 영향을 준다"  
정의  
: 현재 값이 과거 값들의 선형 결합으로 설명되는 모델  
...
t = time   
Yt-1, Yt-2 .... , Yt-p   
- 과거 시점의 값들(Lagged values)     
- t − 1→ 1일 or 1시간 전 (데이터에 따라 1초 전의 값.. 등도 가능)  
- t − 2→ 2일 or 2시간 전  

a1, a2, ...., ap  
- 각 lag 값이 현재 값에 주는 영향력(계수, coefficient)  
- 계수의 크기(=가중치)  
- 클수록 현재 값에 강하게 영향을 줌  
- 작을수록 영향이 적음  
- p가 클수록 “더 긴 과거”까지 참조하는 모델  
  
p의 의미  
- p = 사용하는 과거의 수(lag order)  
- p=1 → 바로 전 시점의 값만 이용  
- p=10 → 10일 or 10 시점 전까지 이용
   
Et   
- 백색잡음(white noise)  
- 예측 불가능한 오차   
- 모델로 설명되지 않는 충격(예: 갑작스러운 사건)  
선형 결합의 의미  
- (과거값 × 영향력)을 모두 더한 구조  
- 즉, 과거 값들의 가중합(weighted sum)  
- 실제 데이터가 과거 흐름을 선형적으로 따르는 패턴을 반영함  
  
예시  
현실 속의 많은 데이터가 관성(inertia)을 가짐    
“어제 주가가 높으면 오늘도 높일 확률이 높다.”   
"하루만에 온도가 급변하지 않음"   
"센서 신호는 이전 패턴과 비슷하게 흘러감"   
-> 즉, 관성이 있는 현실의 많은 데이터를 다루기에 적합함   
  
적용 예: 주가, 온도  
• 음성/진동 신호 등 연속적 패턴이 있는 데이터  
• 정리하자면, AR은 “기억(memory)이 있는 모델”  
• 계수 크기 = 기억의 강도  
• p = 기억의 길이  
• 관성이 있는 시계열(주가, 온도, 센서 데이터)에 적합  
(예시)  
Yt = 0.7Yt-1 + 0.2Yt-1 + Et   
해석  
어제 값이 오늘에 매우 강한 영향 → 0.7  
이틀 전 값은 약하게 영향을 줌 → 0.2  
예측 불가능한 충격은 Et로 표현  


### MA (이동평균) (Moving Average, 이동평균 모델)  
(idea) "오늘 값 = 오늘의 충격 + 과거 충격이 조금씩 남아서 더해진것"   
● 정의  
• 현재 값이 과거 오차(충격)의 누적 효과로 설명되는 모델  
•  =  +  +  + ⋯ +   
• Yt  
• 현재 시점의 값(Current value)  
• 예측하려는 대상  
• MA 모델에서는 과거 오차의 누적 효과로 오늘 값을 설명한다   
###  ARMA (AR + MA)
### ARIMA (차분 + ARMA) (AutoRegressive Integrated Moving Average)
(idea) “추세를 먼저 제거(d차분)한 뒤, AR(과거 값) + MA(과거충격)의 패턴을 적용하는 모델”  
차분  
지금 값 - 전날 값, 전날 값 - 이틀 전값.....   

### RNN, LSTM, GRU  
### RNN(Recurrent Neural Network)  
: 시퀀스 데이터(순서가 있는 데이터)를 처리하기 위한 반복 구조 신경망   
시간에 따른 순서가 있는 입력을 처리하기 위해 사용되는 가장 기본적인 구조  
• (예) 문장, 음성, 주가, 센서값(온도 등), 음악 등  
RNN = 순서를 '기억'하는 가장 기본적인 구조  
'Recurrent = 반복되는' (즉, 순환하는 구조의 Neural Network)   
output_t = f(input_t + hidden_{t-1})(직전 시점의 은닉상태와 현재시점의 입력을 받아 처리함)     
이전 시점으로부터 나온 hidden state를 현재 시점의 입력과 함께 처리함  
같은 가중치 를 모든 시점에서 공유함.  
(비유: 하나의 뇌로 일관성 있게 입력이 들어올 때마다 반복해서 생각한다)  
• h₁ = f(W · x₁ + U · h₀)  
• h₂ = f(W · x₂ + U · h₁)   
• h₃ = f(W · x₃ + U · h₂)    
W: 입력에 곱하는 가중치   
U: 이전 hidden state에 곱하는 가중치  
f: 활성화 함수(tanh, ReLU ...)  
가중치 W와 U는 t(시간)과 무관하게 전부 공유됨    
...
• RNN의 한계  
• 기울기 소실 문제: 역전파 시 기울기가 0에 가까워져 학습이 되지 않는 문제  
• 먼 과거의 정보를 기억하기 어려운 문제: 문장의 앞부분 내용을 뒤에서 활용하기 어려움  
• 대표적인 RNN들  
...
x₁: “옛날에” x₂: “한 소년이” x₃: “살고 있었어요”  
기존 신경망: "옛날에" → 처리 끝 "한 소년이" → 별개로 처리 = 문맥이 끊김!  
RNN: "한 소년이"를 이해할 때 "옛날에"를 기억하고있음 "살고 있었어요"를 이해할 때 앞의 모든 내용을 계속 이어받음  
"옛날에", "한 소년이", "살고 있었어요" 데이터를 입력받고, 각 시점의 RNN  
  
RNN 한시점에서 재귀하듯이 표현하기도 하고 쭉 연결된 것처럼 표현하기도 함. 입력 출력 사이 은닉층이 다음 시점의 노드로 복잡하게 연결된 형태로 나타내기도 함  
  
#### RNN에서의 가중치   
하나의 데이터를 처리할 때, RNN의 가중치들은 다 고정임(이후에 Loss 값을 보고 가중치 업데이트)   
...
→ 새로운 W1을 사용해 연산   
→loss 계산 → W2   
(1) 하나의 문장 처리 중 W, U 등 가중치는 고정 
(2) 문장 끝까지 계산 완료 후 예측 결과와 정답 비교 → loss 계산  
(3) loss 역전파 W, U 업데이트됨  
(4) 다음 문장 업데이트된 W, U로 다시 반복  

#### RNN 입출력
입력 대 출력   
일대다 (one-to-many)   
이미지 캡셔닝 입력: 사진 1장, 출력: "강아지가 공원에서 산책중이다."   
다대일 (many-to-one)  
감정분석 입력: 문장, 출력: 긍/부  
스펨메일 분류 입력: 메일, 출력: 스팸 여부  
다대다 (many-to-many)  
품사 태깅 입력: 문장, 출력: 품사의 종류  
언어번역, 챗봇  

#### 장기 의존성 문제
시퀀스가 길어지면 기울기 소실 현상이 더 심해짐 -> 장기 의존성 문제 발생함  
**기울기 소실(Gradient Vanishing)**  
: 딥러닝에서 역전파를 할 때, 오차를 전달하는 그래디언트가 반복적으로 곱해지며 점점 작아지는 현상  
**장기 의존성 문제(Long-Term Dependency)**    
: 긴 시퀀스에서 앞 부분의 정보를 뒷 부분에서 기억하지 못하는 문제  
RNN은 시간의 흐름에 따라 정보를 순차적으로 받아들이는데, 문장이 길어질수록 초반의 입력이 출력에 영향을 주지 못하게 됨.  
특히 번역, 문맥 해석, 요약처럼 앞에서 입력받은 정보가 중요한 작업에서 문제가 됨  
...
(실습)  
tensorflow - unstack. (batch, time, feature) 한번에 들어가지 않게 즉 time 방향으로 잘라서 각각의 시점별(time step별) 입력으로 변환해주는 함수 -> RNN 입력 데이터 자체를 시간축 단위로 분리하는 기능   

### LSTM(Long Short-Term Memory)  
LSTM은 RNN의 기울기 소실 문제와 장기 의존성 문제를 해결하기 위해 설계된 특수한 구조  
기존 RNN의 hidden state(은닉 상태)에 cell state(셀 상태)를 추가하여 정보를 장기간 저장할 수 있도록 설계함  
여러 gate를 추가하여 정보의 흐름을 정교하게 조절함  
- 필요한 정보는 기억하고   
- 불필요한 정보는 약화시키며   
- 새로운 정보는 선택적으로 저장할 수 있음   
자연어 처리, 시계열 예측, 음성 인식(STT, Speech-To-Text) 등에서 사용됨   

|이름| 의미 | 설명 |
|---|---|---|
|Long-Term| 장기 기억 유지| - RNN은 먼 과거의 정보는 금방 사라지는 문제가있었음 - LSTM은 cell state(셀 상태)를 통해 중요한 정보는오랫동안 유지할 수 있게 됨
|Short-Term| 단기 정보 처리 | - 시퀀스 데이터에는 최신 정보들도 중요함 - LSTM은 최근 입력을 받아들이면서도, 중요도가 낮은 것은 input, 및 candidate gate를 통해 금방 잊을 수 있음| 
|  Memory | 기억 | - hidden state, cell state와 gate 구조들을 통해 기억을저장/삭제/갱신하는 메모리 시스템|  
    
즉, (Vanilla) RNN과는 달리 오래 기억해야 할 것은 기억하고, 금방 잊어도 되는 건 지워버리는 유연한 기억 구조를 가지고 있음  
...
즉 LSTM 은 RNN + cell state(장기 기억) + gate (기억, 반영 여부)  
LSTM은 RNN의 장기 의존성 문제를 해결하였지만, 구조가 복잡하여 학습시간이 오래 걸린다는 단점이 있음   
그래서 LSTM의 성능은 유지하면서도 더 간단하고 학습이 빠른 구조가 필요해짐   
=> GRU(Gated Recurrent Unit)의 등장   

### GRU(Gated Recurrent Unit) 
3(또는 4)개의 gate가 있는 LSTM과 달리, GRU는 단 2개의 gate만 존재함
GRU의 두 gate들이 정보의 흐름을 정교하게 조절함
Update Gate: 기억을 얼마나 업데이트할까?
- (과거 정보와 새 정보의 비율 조절)
Reset Gate: 이전 기억을 얼마나 지워버릴까?
- (과거 정보를 얼마나 적게 쓸지 조절)
  2개의 state(hidden state, cell state)를 사용하던 LSTM과 달리, GRU는 Hidden state만 사용함

####  RNN, LSTM, GRU 성능 비교  
• RNN 셀의 훈련 데이터셋에 대한 정확도가 가장 높으나, 검증데이터셋에 대한 정확도는 다른 모델에 비해 조금 낮음  
• 훈련 데이터셋과 테스트 데이터셋의 정확도 및 수행 시간을모두고려한다면 RNN 계층의 성능이 가장 좋다고 할 수도 있음(물론수행 시간이 중요하지 않다고 판단할 때는 GRU를 선택할수도있음)  
...
• 이전 시점의 데이터뿐만 아니라, 이후 시점의 데이터도 함께활용하여 출력 값을 예측하고자 하는 것이 양방향 RNN(bidirectional RNN)  
실시간 대화 -> 양방향 RNN X 불가  

구조
- 과거의 정보(forward)와 미래의 정보(backward)를 동시에 사용하는 순환 신경망  
- 현재 단어를 예측할 때, 지나온 단어(과거) 정보만 사용하였던 RNN과 달리, 현재 단어의 앞뒤 문맥을 모두 활용하여 의미 해석 정확도를 향상하는 기법.
- (예) 나는 은행에서 돈을 찾았다. 은행의 의미를 확인하려면, 앞의 단어, 뒤의 단어가 모두 필요함
=**문맥을 파악** 하는 데에 양방향 RNN이 유리함

## 12주차 자연어 처리
  
자연어 처리 우리가 일상생활에서 사용하는 언어의 의미를 분석하여 컴퓨터가 처리할 수 있도록 하는 과정  
...
현재 자연어 처리 완성도  
완성도 높음: 스팸 처리(spam detection), 맞춤법 검사(spell checking), 단어 검색(keyword search), 객체 인식(named entity recognition)    
완성도 낮음: 질의응답(Q&A), 요약(summarization), 유사 단어 바꿔쓰기(paraphase), 대화(dialog)  
...
- 불용어(stop words): 문장 내에서 많이 등장하는 단어 분석과 관계없으며, 자주 등장하는 빈도 때문에 성능에 영향을 미치므로 사전에 제거해주어야 함(영어 예시: “a”, “the”, “she”, “he”)  
- 어간 추출(stemming): 단어를 기본 형태로 만드는 작업  ex) ‘consign’, ‘consigned’, ‘consigning’, ‘consignment' -> ‘consign’  
- 품사 태깅(part-of-speech tagging): 주어진 문장에서 품사를 식별하기 위해 붙여주는 태그(식별 정보)를 의미  
Det: 한정사 a, the, that, this, these, my, your  
Noun: 명사 cat, sofa  
Verb: 동사 is, run, study  
Prep: 전치사 on, in, of
품사 태깅은 NLTK(Natural Langauge ToolKit)를 이용할 수 있음  
NLTK    
VBZ: 동사, 동명사 또는 현재 분사 is, likes  
PRP: 인칭 대명사(PP) I, you, she  
JJ: 형용사 big, beautiful, small  
VBG: 동사, 동명사 또는 현재 분사 running, eating, studying   
NNS: 명사, 복수형 dogs, cars  
CC: 등위 접속사 and, or, but   
  
KoNLPy  
KoNLPy는 한국어 처리를 위한 파이썬 라이브러리  
...
즉, 텍스트를 수치로 바꾸고, 토픽을 추출하는 데에 특화된 라이브러리임   
- 토픽 모델링: 문서들의 주제를 자동으로 추출하는 기술   
...
**절차**  
문장 -> 전처리 확인 토큰화 -> 단어 색인 -> 불용어 제거 -> 축소된 단어 색인 -> 어간 추출  
...
예를 들어 어간 추출은 다음과 같이 사용 Automates, automatic, automation → automat  
  
표제어 추출은 단어가 문장 속에서 어떤 품사로 쓰였는지 고려하기 때문에 품사가 같아야 사용 가능  
...
MonthlyIncome은 0~10000의 범위를 갖지만, RelationshipSatisfaction은 0~5의 범위를 갖음  
...
- StandardScaler()[-> Z-score ]: 각 특성의 평균을 0, 분산을 1로 변경하여 특성의 스케일을 조정 | x - 평균 / 표준편차    
- RobustScaler(): 평균과 분산 대신 중간 값(median)과 사분위수 범위(InterQuartile Range, IQR)를 사용   
- MinMaxScaler(): 모든 특성이 0과 1 사이에 위치하도록 스케일을 조정  
이때 이상치의 데이터가 있을 경우 반환된 값이 매우 좁은 범위로 압축될 수 있음 즉, 이상치에 매우 민감할 수 있기 때문에 주의해야 함   
- MaxAbsScaler(): 절댓값이 0~1 사이가 되도록 조정  
즉, 모든 데이터가 -1~1의 사이가 되도록 조정하기 때문에 양의 수로만 구성된 데이터는 MinMaxScaler()와 유사하게 동작 또한, 큰 이상치에 민감하다는 단점이 있음   
...
임베딩 방법에 따라...   
• 희소 표현 기반 임베딩 - one-hot embedding  
• 횟수 기반 임베딩 - CountVectorizer, TF-IDF  
• 예측 기반 임베딩 - Word2Vec, CBOW, Skip-gram, FastText  
• 횟수/예측 기반 임베딩 - GloVe  
...
• 다시말해 단어 N개를 각각 N차원의 벡터로 표현하는 방식으로, 단어가 포함되어 있는 위치에 1을 넣고 나머지에는 0 값을 채움  
• 예를 들어 딕셔너리에 [calm, fast, cat] 같은 값이 있다면 fast를 표현하는 벡터는 [0, 1, 0]이 됨  
치명적인 단점  
1. 단어간의 관계를 파악하기 힘듦  
수학적인 의미에서 원-핫 벡터들은 하나의 요소만 1 값을 갖고 나머지는 모두 0인 희소 벡터(sparse vector)로 표현됨  
이때 두 단어 사이의 관계성을 구하면, 관계성(유의어, 반의어) 없이 서로 독립적(independent)인 관계가 됨  
2.  ‘차원의 저주(curse of dimensionality)’ 문제가 발생함   
하나의 단어를 표현하는 데 말뭉치(corpus)에 있는 수만큼 차원이 존재하기 때문에 복잡해짐  
예를 들어 단어 10만 개를 포함한 데이터셋에 원-핫 인코딩 배열을 구성한다면 그 차원 개수는 10만 개  
  
원-핫 인코딩에 대한 대안으로 신경망에 기반하여 단어를 벡터로 바꾸는 방법론들이 주목을 받고 있음  
• 예를 들어 워드투벡터(Word2Vec), 글로브(GloVe), 패스트텍스트(FastText) 등이 대표적인 방법론임  

### 횟수 기반 임베딩   
횟수 기반은 단어가 출현한 빈도를 고려하여 임베딩하는 방법  
• 대표적으로 카운터 벡터와 TF-IDF가 있음   

Counter Vector(카운터 벡터)  
카운터 벡터는 문서 집합에서 단어를 토큰으로 생성하고 각 단어의 출현 빈도수를 이용하여 인코딩해서 벡터를 만드는 방법  
즉, 토크나이징과 벡터화를 동시에 하는 방법  
카운터 벡터는 사이킷런의 CountVectorizer()를 사용하여 코드로 구현할 수 있음  
CountVectorizer()는 다음 작업을 할 수 있음  
1. 문서를 토큰 리스트로 변환  
...
TF = 단어 빈도(문서 안에서 해당 단어가 얼마나 많이 등장하였는가?)  
IDF = 역문서 빈도  
많은 문서에서 흔하게 나타나는 단어의 중요도를 낮추는 역할을 함  
• 문서마다 발견되는 흔한 단어(a, the, 그리고, 은)의 가중치를 낮추어 문서의 특정 단어를 더 잘 반영함   
• 그래서 검색엔진, 키워드 추출, 문서 분류에 효과적  
다음 상황에서 사용  
...
이때 모든 분할된 텍스트는 한 쌍의 대상 단어와 컨텍스트로 네트워크에 입력됨  

CBOW(Continuous Bag Of Words)  
CBOW는 주변 단어(문맥)를 보고, 중심 단어를 예측하도록 학습해, 단어를 벡터로 만드는(임베딩하는) 방법  
• 예를 들어 “calm cat slept on the sofa”라는 문장이 있을 때, “calm cat ___ on the sofa”라는 문맥이 주어지면 “slept”를 예측하는 것이 CBOW의 방식  
• the, cat, on, the, sofa 벡터들의 평균을 구하여 중심에 오는 단어인 slept를 예측함  
• 이렇게 반복하면서 단어의 의미를 벡터 공간으로 나타내는 법을 학습함  
• 즉, 단어의 의미를 수치화하고, 비슷한 단어를 비슷한 벡터로 만드는 역할을 함  

### Transformer
트랜스포머 어텐션
어텐션(attention)은 주로 언어 번역에서 사용되기 때문에 인코더와 디코더 네트워크를 사용  
즉, 입력에 대한 벡터 변환을 인코더(encoder)에서 처리하고 모든 벡터를 디코더로 보냄  
이렇게 모든 벡터를 전달하는 이유는 시간이 흐를수록 초기 정보를 잃어버리는 기울기 소멸 문제를 해결하기 위해서임  
모든 벡터가 전달되기 때문에 행렬 크기가 굉장히 커지는 단점이 있는데, 이것을 해결하기 위해 소프트맥스 함수를 사용하여 가중합을 구하고 그 값을 디코더에 전달  
가중합만 전달되었더라도 정보를 많이 전달받은 디코더는 부담일 수밖에 없음  
디코더는 은닉 상태에 대해 중점적으로 집중(attention)해서 보아야 할 벡터를 소프트맥스 함수로 점수를 매긴 후 각각을 은닉 상태의 벡터들과 곱함  
이 은닉 상태를 모두 더해서 하나의 값으로 만듦    
즉, 어텐션은 모든 벡터 중에서 꼭 살펴보아야 할 벡터들에 집중하겠다는 의미  

### 한국어 임베딩 (실습)
영어와 다르지 않다는 것을 확인할 수 있었음   
즉, 자연어 처리를 위한 임베딩 방법만 알고 있다면 언어와 상관없이 단어/문장에 대한 임베딩을 진행하며, 모델을 생성하고 훈련시킨 후 예측 및 분류를 수행할 수 있음  
